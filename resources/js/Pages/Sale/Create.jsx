...
export default function Create({ products }) {
    ...
    const submit = (e) => {
        ...
        const saleItems = items
            .filter(item => {
                const hasProduct = item.product_id && item.product_id !== "" && item.product_id !== "0";
        ...
        if (saleItems.length === 0) {
            const emptyProducts = items.filter(i => !i.product_id || i.product_id === "");
        ...
        console.log("🔍 Submitting sale:", {
            ...
            items: saleItems,
            total: saleItems.reduce((sum, i) => sum + (i.quantity * i.unit_price), 0)
        });
        ...
        post('/sales', {
            ...
            payment_method: data.payment_method,
            items: saleItems,
            total_amount: saleItems.reduce((sum, i) => sum + (i.quantity * i.unit_price), 0),
        }, {
            preserveScroll: true,
            onSuccess: () => {
                                ...
                                <div key={index} className="border border-gray-200 rounded-lg p-4 mb-4 bg-gray-50">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                        ...
                                        <div className="md:col-span-2">
                                            ...
                                            <select
                                                ...
                                                className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                                                    !item.product_id ? 'border-red-400 bg-red-50' : 'border-gray-300'
                                                }`}
                                                required
                                            >
                                                <option value="">⚠️ Select a product...</option>
                                                {products.map(p => (
                                                    <option key={p.id} value={p.id} disabled={p.current_quantity <= 0}>
                                                        {p.name} ({p.current_quantity} in stock) - Br {p.unit_sell_price}
                                                    </option>
                                                ))}
                                            </select>
                                        ...
                                        <div>
                                            ...
                                            />
                                            {availableStock > 0 && (
                                                <p className="text-xs text-gray-500 mt-1">{availableStock} available</p>
                                            )}
                                        </div>
                                        
                                        {/* Unit Price - in Ethiopian Birr */}
                                        <div>
                                            <label className="block text-xs font-medium text-gray-600 mb-1">Price (Br) *</label>
                                            <input
                                                type="number"
                                                step="0.01"
                                                min="0"
                                    ...
                                    <div className="mt-3 text-right">
                                        ...
                                        <span className="font-semibold text-gray-900">Br {calculateSubtotal(item)}</span>
                                    </div>
                                </div>
                            );
                        })}
                    ...
                    <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                            <div>
                                ...
                                <span className="text-2xl font-bold text-green-600">Br {calculateTotal()}</span>
                            </div>
                            ...
                            <div className="flex gap-3">
                                ...
                                <button
                                    type="submit"
                                    disabled={processing || items.every(i => !i.product_id)}
